...
export default function CampaignResultsPage() {
    ...
    if (loading) return (
        <div className="flex items-center justify-center h-full">
            <Activity className="animate-spin text-lf-accent" size={32} />
        </div>
    )
    ...
    if (!campaign) return (
        <div className="p-20 text-center flex flex-col items-center gap-4">
            ...
            <button onClick={() => router.push('/campaigns')} className="text-lf-accent underline">Voltar</button>
        </div>
    )
            ...
            <main className="flex-1 overflow-y-auto p-8">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <StatCard icon={<Zap size={20} />} label="Ações Realizadas" value={campaign.totalCompleted} color="text-lf-accent" />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            <div className="bg-white border border-lf-border rounded-2xl p-6 shadow-sm">
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <span className="font-bold text-lf-text2 uppercase tracking-widest text-sm">
                                        {campaign.status === 'active' ? 'Em execução' : campaign.status}
                                    </span>
                                </div>
                                <div className="space-y-4">
                                    <div className="flex justify-between text-xs">
                                        <span className="text-lf-text4">Início:</span>
                                        <span className="font-semibold">{new Date(campaign.createdAt).toLocaleDateString()}</span>
                                    </div>
                                    <div className="flex justify-between text-xs">
                                        <span className="text-lf-text4">Expira em:</span>
                        ...
                        <div className="lg:col-span-2">
                            <div className="bg-white border border-lf-border rounded-2xl p-6 shadow-sm h-full flex flex-col">
                                ...
                                <div className="flex-1 space-y-6 overflow-y-auto">
                                    {events.length === 0 ? (
                                        <div className="h-40 flex flex-col items-center justify-center gap-3 text-lf-text4">
                                            <Clock size={32} strokeWidth={1} />
                                            <span className="text-xs">Aguardando as primeiras interações...</span>
                                        </div>
                                    ) : (
                                        events.map(event => (
                                            <div key={event.id} className="flex gap-4 items-start pl-2 border-l-2 border-lf-s1 relative">
                                                <div className="absolute -left-[9px] top-1 h-4 w-4 rounded-full bg-white border-4 border-lf-accent" />
                                                <div className="flex-1">
                                                    <div className="flex justify-between items-center mb-1">
                                                        <span className="text-xs font-bold text-lf-text2">{event.eventType}</span>
                                                        <span className="text-[10px] text-lf-text4">{new Date(event.createdAt).toLocaleTimeString()}</span>
                                                    </div>
                                                    <p className="text-xs text-lf-text3">Ação concluída com sucesso para o lead</p>
                                                </div>
                                            </div>
                                        ))
                                    )}
...
function StatCard({ icon, label, value, color }: { icon: any, label: string, value: any, color: string }) {
    return (
        <div className="bg-white border border-lf-border rounded-2xl p-6 shadow-sm flex flex-col gap-1 transition-transform hover:-translate-y-1">
            <div className={cn("mb-2", color)}>{icon}</div>
            <span className="lf-label text-[10px] text-lf-text4 uppercase tracking-wider">{label}</span>
            <span className={cn("font-bricolage text-2xl font-bold", color)}>{value}</span>
        </div>
    )
}
